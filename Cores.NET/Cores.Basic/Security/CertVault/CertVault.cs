...
public class CertVault : AsyncServiceWithMainLoop
{
    ...
    public CertVault(DirectoryPath baseDir, CertVaultSettings? defaultSettings = null, CertificateStore? defaultCertificate = null, TcpIpSystem? tcpIp = null, bool isGlobalVault = false, Func<CertVault, CertificateStore>? defaultCertificateGenerator = null)
    {
        try
        {
            ...
            this.AutoGeneratingRootCA = DevTools.CoresDebugCACert_20221125.PkiCertificateStore;

            this.AutoGeneratingRootCA2 = DevTools.CoresDebugCACert_20221125.PkiCertificateStore;
    ...
    void InternalReload()
    {
        ...
        if (this.Settings.NonAcmeEnableAutoGenerateSubjectNameCert && this.Settings.UseAcme == false)
        {
            ...
            CertificateStore newCa = DevTools.CoresDebugCACert_20221125.PkiCertificateStore;
            ...
            defaultCert = defaultCertPath.ReadAndParseDataFile(ReadParseFlags.ForceInitOnParseError,
                ...
                () =>
                {
                    ...
                    else
                    {
                        ...
                        {
                            PkiUtil.GenerateRsaKeyPair(2048, out PrivKey key, out _);
                            Certificate cert = new Certificate(key, new CertificateOptions(PkiAlgorithm.RSA, cn: Consts.Strings.DefaultCertCN + "_" + Env.MachineName, c: "US", expires: Util.MaxDateTimeOffsetValue, type: CertificateOptionsType.ServerCertificate));
                ...
                if (CheckFqdnAllowedForAutoGenerateNonAcmeSubjectNameCertFqdnList(hostname) && PathParser.Windows.IsSafeFileName(hostname))
                {
                    ...
                    else
                    {
                        ...
                        if (forceRewrite || path.IsFileExists() == false)
                        {
                            ...
                            var ca = this.AutoGeneratingRootCA;
                    ...
                    if (PathParser.Windows.IsSafeFileName(hostname))
                    {
                        ...
                        else
                        {
                            ...
                            if (forceRewrite || path.IsFileExists() == false)
                            {
                                ...
                                var ca = this.AutoGeneratingRootCA;
                                //var ca = new CertificateStore(Lfs.ReadDataFromFile(@"c:\tmp\ca.p12").Span);
                                //var ca = new CertificateStore(Lfs.ReadDataFromFile(@"C:\tmp\221124cert\01_easy_root_ca.cer").Span, Lfs.ReadDataFromFile("C:\\tmp\\221124cert\\01_easy_root_ca.key").Span);
