...
namespace IPA.Cores.Basic
{
    ...
    ref partial struct Packet
    {
        ...
        public byte[] ToArray() => Buffer.Span.ToArray();

        public byte[] _DebugItemsSlow { get => ToArray(); }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public Packet(ReadOnlySpan<byte> initialContents)
        {
            Buffer = new ElasticBuffer();
            Buffer.InsertHead(initialContents);
            PinHead = 0;
            PinTail = initialContents.Length;
        ...
        public unsafe ref readonly T AsReadOnlyStruct<T>(int pin, int? size = null) where T : struct
        {
            ...
            fixed (void* ptr = this.Span.Slice(pin))
    ...
    readonly unsafe struct PacketPin<T> where T : struct
    {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool IsEmpty(ref Packet packet)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public bool IsFilled(ref Packet packet)
            => !IsEmpty(ref packet);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public PacketPin(int pin, int? headerSize = null, int maxPacketSize = int.MaxValue)
        {
            this.Pin = pin;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public unsafe ref readonly T RefValueRead(ref Packet packet)
        {
            return ref packet.AsReadOnlyStruct<T>(this.Pin, this.HeaderSize);
        }
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public PacketPin<TNext> GetNextHeader<TNext>(ref Packet packet, int? size = null) where TNext : struct
        {
            return packet.GetHeader<TNext>(this.Pin + this.HeaderSize, size, this.PayloadSize(ref packet));
        }
    }
