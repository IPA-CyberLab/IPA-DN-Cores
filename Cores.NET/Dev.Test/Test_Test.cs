...
namespace IPA.TestDev
{
    ...
    static class TestClass
    {
        ...
        public static class Test_DeepClone
        {
            public static volatile int ConstructorCount;
            ...
            public class ElementClass
            {
                public ElementClass()
                {
                    Test_DeepClone.ConstructorCount++;
                }
            ...
            static void DoTest(DeepCloneMethod method)
            {
                Test_DeepClone.ConstructorCount = 0;
                ...
                if (Test_DeepClone.ConstructorCount != 0)
                    throw new CoresLibException("Test_DeepClone.ConstructorCount != 0");

                if (object.ReferenceEquals(a, b))
                    throw new CoresLibException("a == b");

                if (a.Dict.Count != b.Dict!.Count)
                    throw new CoresLibException("a.Dict.Count != b.Dict!.Count");
                ...
                for (int i = 0; i < a.Dict.Count; i++)
                {
                    ...
                    if (object.ReferenceEquals(e1, e2))
                        throw new CoresLibException("e1 == e2");

                    if (e1.Int1 != e2.Int1)
                        throw new CoresLibException("e1.Int != e2.Int");

                    if (e1.Str1 != e2.Str1)
                        throw new CoresLibException("e1.Str != e2.Str");

                    if (e1.Str1._IsEmpty())
                        throw new CoresLibException("e1.Str1 is empty");
                    ...
                    if (e1_int2 != e2_int2)
                        throw new CoresLibException("e1_int2 != e2_int2");

                    if (e1_str2 != e2_str2)
                        throw new CoresLibException("e1_str2 != e2_str2");

                    if (e1_str2._IsEmpty())
                        throw new CoresLibException("e1_str2 is empty");

                    if (object.ReferenceEquals(e1.Root, a) == false)
                        throw new CoresLibException("e1.root != a");

                    if (object.ReferenceEquals(e2.Root, b) == false)
                        throw new CoresLibException("e2.root != b");
                }
            }

            public static void Test()
            {
                DoTest(DeepCloneMethod.BinaryFormatter);
                DoTest(DeepCloneMethod.DeepCloner);
            }
        }
