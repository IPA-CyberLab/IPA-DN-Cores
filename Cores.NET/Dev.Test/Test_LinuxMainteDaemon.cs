...
class LinuxMainteDaemon : Daemon
{
    LinuxMainteDaemonApp? app = null;

    public LinuxMainteDaemon() : base(new DaemonOptions("LinuxMainteDaemon", "LinuxMainteDaemon Service", true))
    {
    }

    protected override async Task StartImplAsync(DaemonStartupMode startupMode, object? param)
    {
        Con.WriteLine("LinuxMainteDaemon: Starting...");

        app = new LinuxMainteDaemonApp();

        await Task.CompletedTask;

        try
        {
            Con.WriteLine("LinuxMainteDaemon: Started.");
        }
        catch
        {
    ...
    {
        Con.WriteLine("LinuxMainteDaemon: Stopping...");
        ...
        Con.WriteLine("LinuxMainteDaemon: Stopped.");
    }
}
...
{
    [ConsoleCommand(
        "Start or stop the LinuxMainteDaemon daemon",
        "LinuxMainteDaemon [command]",
        "Start or stop the LinuxMainteDaemon daemon",
...
winuninstall - Uninstall the daemon as a Windows service.")]
    static int LinuxMainteDaemon(ConsoleService c, string cmdName, string str)
    {
        return DaemonCmdLineTool.EntryPoint(c, cmdName, str, new LinuxMainteDaemon(), new DaemonSettings());
    }
