...
namespace IPA.DataVaultServer
{
    class DataVaultServerAppMain
    {
        static int Main(string[] args)
        {
            ...
            CoresLib.Init(new CoresLibOptions(CoresMode.Daemon, "DataVaultServer", DebugMode.Debug, defaultPrintStatToConsole: false, defaultRecordLeakFullStack: false), args);

            try
            {
                ret = ConsoleService.EntryPoint(Env.CommandLine, "DataVaultServer", typeof(DataVaultServerAppMain));
            }
            finally
            {
                CoresLib.Free();
        ...
        [ConsoleCommand(
            "IPA.Cores DataVaultServer Program",
            "[/IN:infile] [/OUT:outfile] [/CSV] [/CMD command_line...]",
            "This is the DataVaultServer Program.",
        ...
        static int DataVaultServer(ConsoleService c, string cmdName, string str)
        {
            ...
            while (cs.DispatchCommand(cmdline, "DataVaultServer>", typeof(DataVaultServerCommands), null))
            {
